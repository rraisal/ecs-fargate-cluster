...
resource "aws_codepipeline" "ecs_pipeline" {
  ...
  stage {
    ...
    action {
      category = "Build"
      configuration = {
        "ProjectName" = join("-", [var.environment, "build"])
      }
      input_artifacts = [
        "SourceArtifact",
      ]
...
resource "aws_codebuild_project" "ecs_build" {
  badge_enabled  = false
  build_timeout  = 60
  name           = join("-", [var.environment, "build"])
  queued_timeout = 480
  service_role   = aws_iam_role.codebuild.arn
  artifacts {
    encryption_disabled    = false
    name                   = "ecs-build-${var.environment}"
...
resource "aws_codedeploy_deployment_group" "ecs" {
  ...
  deployment_group_name  = "deployment-group-${aws_ecs_service.main.name}"
